...
Tower.SnappingCursor = lat.SnapCursor.extend({
	...
	draw: function (x, y) {
		...
		if (typeof ig.game.clickType === 'function') {
			...
			if (preview && preview.image) {
				...
				var proto = ig.game.clickType.prototype;
				if (proto && proto.drawRange) {
					proto.drawRange.call({
						size: proto.size,
						pos: snap,
						range: proto.range
					});
		...
		if (this._dcCt > 0) {
			this.font.draw(this._dcCt, 80, 8);
		}
	},
	...
	doubleClick: function (e) {
		if (e.snap.x === 0 && e.snap.y === 0) {
			Tower.Action.Delay._toggleQuickDelay();
		}
		else if (e.snap.x === 80 && e.snap.y === 0) {
			this._dcCt++;
			clearTimeout(this._dcTimeout);
			this._dcTimeout = setTimeout(function () {
				Tower.jumpTo(this._dcCt);
				this._dcCt = 0;
			}.bind(this), 1000);
		}
		else if (e.snap.x === 0 && e.snap.y === 104) {
			Tower.motherlode();
		}
	}
});
