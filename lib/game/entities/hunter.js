...
EntityHunter = EnemyUnit.extend({
	...
	click: function (e) {
		if (e.key !== ig.KEY.MOUSE1) return;
		if (this.recoiling) return;
		this.receiveDamage(3.4, ig.game.cursor);
		return true;
	},
	...
	collideWith: function (a) {
		...
		else if (a instanceof EntityWall) {
			a.receiveDamage(1);
		}
	},
	...
	mouseEnter: function () {
		...
		ig.game.cursor.enemiesAtCursor++;
