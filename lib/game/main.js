	...
	click: function (e) {
		...
		if (e.key === ig.KEY.MOUSE1) {
			if (ig.game.clickType === 'delete') {
				...
				if (target) {
					if (target.hbar) {
						if (target.health < target.hbar.max) {
							var incr = target.hbar.max/10;
							var maxPrice = target.value * 0.6 / 10;
							var diff = Math.min(target.health + incr, target.hbar.max) - target.health;
							var price = Math.ceil(diff.map(0, incr, 0, maxPrice));

							if (Tower.treasury.funds >= price) {
								Tower.treasury.funds -= price;
								target.health += diff;
							}
						}
					}
